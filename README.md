...
3. GUI 下部のボタンで制御モードを選択:
   ...
   - **Bilateral** — バイラテラル制御（remote + EMS 力フィードバック）
...
| **controller** | `inc/controller.h`, `src/controller.cc`                                   | 制御則の実装（idle / DA_check / remote / Record / Bilateral） |
...
| キー                | 型    | 説明                                             |
| ------------------- | ----- | ------------------------------------------------ |
| `force_threshold`   | float | f_dis がこの値以下なら EMS 出力 0V（ノイズ除去） |
| `force_max`         | float | この力で最大電圧に達する                         |
| `voltage_threshold` | float | EMS 回路が反応し始める最低電圧 V_th [V]          |
| `voltage_max`       | float | EMS 回路の最大電圧 [V]（DA ch1 出力上限）        |

Bilateral モードで使用。詳細は [docs/guides/ems.md](docs/guides/ems.md) を参照。
...
| **Phase 3** | バイラテラル制御 — 力センシング（RFOB）、EMS 力フィードバック、双方向制御                        | 進行中 |
